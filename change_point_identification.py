...
def sample_configurations(
    ...
    '''
    Select a sample of configurations either using randoms sampling or t-wise sampling
    ...
    degree : int
        degree up to which t-wise sampling is conducted

    randomized : bool
        enable random sampling (default is False)

    size : bool
        size of random sample (only considered if randomized is True).

    Returns
    -------

    List( (#configs, #options) with configurations
    '''
    xs = []

    # random sampling mode
    if randomized:
        return np.random.choice(np.arange(2), size=(size, n_options))

    # t-wise sampling mode (including option-wise sampling)
    for degree in range(1, degree + 1):
        for config in itertools.combinations(np.arange(n_options), degree):
            cfg = np.full([n_options, ], 0)
    ...
    for a, b in itertools.combinations(perf.keys(), 2):
        ...
        if 1:
            if any([diff > threshold_a, diff > threshold_b]):
                # print(threshold_a, perf[a][0], threshold_b, perf[a][0])
                cpl[min(a, b): max(a, b)] += 1 / np.abs(a - b) ** 2
...
def query_configurations(
    ...
    '''
    if max_distance == -1:
        max_distance = n_options

    assert min_distance < max_distance
    assert min_distance > 0 and max_distance >= 1
    assert max_distance <= n_options
...
class ChangePointLearner:
    ...
    def caching(self, evapo_rate=0.3, n_rounds=3):
        ...
        print("-------------------------")
        for key in self.cached_solutions:
            print(key, self.cached_solutions[key])

    def stop(self):
        # print(self.cached_solutions_keys)
...
if __name__ == "__main__":
    ...
    for i in range(30):
        if not learner.stop():
            ...
            # learner.candidate_solutions()
            # print(learner.exploit_commits())
            # print(learner.explore_commits())
            print("bener")
            learner.acquire_configurations()
            print("zeit")
            sample = learner.next_sample()
            # print(learner.candidate_solutions)
            print("nd")
            learner.caching()
